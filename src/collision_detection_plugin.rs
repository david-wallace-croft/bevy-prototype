...
impl CollisionDetectionPlugin {
  fn apply_collision_damage(
    ...
    mut collision_event_message_reader: MessageReader<CollisionEvent>,
    mut health_query: Query<&mut Health>,
  ) {
    for &CollisionEvent {
      collided_entity,
      entity,
    } in collision_event_message_reader.read()
    {
  ...
  fn handle_collisions<T: Component>(
    mut collision_event_message_writer: MessageWriter<CollisionEvent>,
    ...
    for (entity, collider) in query.iter() {
      for &collided_entity in collider.colliding_entities.iter() {
        ...
        collision_event_message_writer
